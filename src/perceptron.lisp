...
;;;; apprentissage par la règle de Widrow-Hoff (règle delta).
;;;;
;;;; Portage dans neuromuse du perceptron original de Frédéric Voisin,
...
(defclass perceptron (ann)
  ...
  (:documentation
   "Perceptron simple (Rosenblatt), sans couche cachée ni biais, comme en 1999.
...
(defmethod print-object ((self perceptron) stream)
  (format stream "<perceptron ~S: ~D input~:P, ~D output~:P>"
          (string (name self)) (in-size self) (out-size self))
  (values))

(defun init-perceptron-net (in-size out-size &key (range 2.0))
...
OUT-SIZE poids tirés dans [-range/2, range/2[ (range 2.0 = [-1, 1[ comme en 1999)."
  (make-listarray in-size out-size
                  #'(lambda () (- (random (float range)) (/ range 2)))))

(defmacro make-perceptron (name in out &key (range 2.0))
...
(par défaut (input self)), avant la fonction de transfert."
  (let ((input (or in (input self))))
    ...
    (perceptron-net-activity (noise self (net-temp self)) input)))
...
Renvoie le nombre de sorties erronées AVANT correction."
  (let* ((input (or in (input self)))
         ...
         (output (run-perceptron self :in input))
         (l (learn-fact self))
    ...
    (when (verbose self)
      (format t "~&~S : ~S <<< ~S, ~D erreur~:P" input goal output e))
    e))
...
(defmethod clear ((self perceptron))
  (setf (previous self) (net self)
        (net self) (init-perceptron-net (in-size self) (out-size self))
        (epoch self) 0
